...
export type PiResourceLoaderOptions = Partial<
  Omit<
...
export interface PiSessionSettings {
  ...
  /**
   * Use a specific Pi agent directory instead of the adapter's isolated temp
   * directory. Point this at Pi's normal agent dir to reuse `auth.json`,
   * `models.json`, settings, and discovered resources intentionally.
   */
  ...
  /** Convenience extension knobs merged into `resourceLoaderOptions`. */
  readonly extensionFactories?: ReadonlyArray<ExtensionFactory>;
  readonly additionalExtensionPaths?: ReadonlyArray<string>;

  /**
   * Extension-registered tool names to activate. The stock adapter only
   * activates harness built-ins plus AI SDK user tools.
   */
  readonly extensionToolNames?: ReadonlyArray<string>;

  /** Activate every registered Pi tool after the raw SDK session starts. */
  readonly activateAllExtensionTools?: boolean;

  /** Opt-in resource discovery. Defaults keep the stock adapter's safe mode. */
  readonly enableExtensions?: boolean;
  readonly enableThemes?: boolean;
  readonly enablePromptTemplates?: boolean;
  readonly allowGlobalSkills?: boolean;
}
...
export async function createPiSession(
  ...
  const hostAgentDir = input.settings.agentDir
  ...
  async function rebuildPiSession(
    ...
    const activateAllExtensionTools =
      input.settings.activateAllExtensionTools ?? false;
    ...
    const { session } = await createAgentSession({
      ...
      resourceLoader,
      customTools,
      ...(activateAllExtensionTools ? {} : { tools: explicitToolNames }),
      ...(input.settings.thinkingLevel
        ? { thinkingLevel: input.settings.thinkingLevel }
        : {}),
      ...(resolvedModel ? { model: resolvedModel } : {}),
    });
    if (activateAllExtensionTools) {
